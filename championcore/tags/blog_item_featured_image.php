...
/**
 * blog item featured image (if any)
 */
class BlogItemFeaturedImage extends Base {
	...
	public function generate_html (array $params = [], array $tag_runner_context = [], string $tag_content = '') : string {
		...
		$probe = \trim( $arguments['blog_item']->id );
		
		$result = $logic_featured_image->process(
			[
				'blog_id'            => $probe,
				'blog_item_location' => $arguments['blog_item']->location
			]
		);
		
		if ($result->url !== false) {
			
			$result = "<img alt=\"teaser-image\" src=\"{$result->url}\" />";
		} else {
			# noop
			$result = '';
		}
		...
		# wrap
		$result = "<div class=\"blog-item-grid-item tag-blog-item-featured-image\">{$result}</div>\n";
