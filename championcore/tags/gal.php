...
/**
 * display a gallery in the media directory
 */
class Gal extends Base {
	...
	public function generate_html (array $params = [], array $tag_runner_context = [], string $tag_content = '') : string {
		...
		\championcore\pre_condition(    \strlen($arguments['gallery_directory']) > 0);
		...
		\championcore\pre_condition(    \strlen($arguments['number_of_images']) > 0);
		...
		\championcore\pre_condition(    \strlen($arguments['popup_all_images']) > 0);
		...
		# render template
		$view = new \championcore\View( \championcore\get_configs()->dir_template . '/tags/gal.phtml' );
