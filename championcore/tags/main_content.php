...
class MainContent extends Base {
	...
	public static function execute_tag (array $tag_vars, array $tag_runner_context = [], string $tag_content = '') : string {
		...
		$tag = new \championcore\tags\MainContent();
	...
	public function generate_html (array $params = [], array $tag_runner_context = [], string $tag_content = '') : string {
		...
		$result =
			isset(\championcore\get_context()->parsedown)
			? \championcore\get_context()->parsedown->text( \championcore\get_context()->content )
			: \championcore\get_context()->content;
