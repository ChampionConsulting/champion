...
/**
 * display a gallery in the media directory - using masonry
 */
class GalMasonry extends Base {
	...
	public static function execute_tag (array $tag_vars, array $tag_runner_context = [], string $tag_content = '') : string {
		...
		$tag = new \championcore\tags\GalMasonry();
	...
	public function generate_html (array $params = [], array $tag_runner_context = [], string $tag_content = '') : string {
		...
		\championcore\get_context()->theme->js_body->add( CHAMPION_BASE_URL . "/championcore/asset/js/tag/gal_masonry.js", [], 'gal_masonry' );
		
		# render template
		$view = new \championcore\View( \championcore\get_configs()->dir_template . '/tags/gal_masonry.phtml' );
