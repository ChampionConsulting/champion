...
class TrackingCode extends Base {
	...
	public static function execute_tag (array $tag_vars, array $tag_runner_context = [], string $tag_content = '') : string {
		...
		$tag = new \championcore\tags\TrackingCode();
	...
	public function generate_html (array $params = [], array $tag_runner_context = [], string $tag_content = '') : string {
		...
		$http_referrer = (empty($_SERVER['HTTP_REFERER']) ? 'none' : $_SERVER['HTTP_REFERER']);
		
		$result = 
			'<script src="'
			. \championcore\wedge\config\get_json_configs()->json->path
			. '/'
			. \championcore\wedge\config\get_json_configs()->json->admin
			. '/inc/tracker.php?uri='
			. \urlencode($_SERVER['REQUEST_URI'])
			. '&ref='
			. \urlencode($http_referrer)
			. '"></script>';
