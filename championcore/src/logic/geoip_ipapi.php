...
class GeoIpIpApi extends Base {
	...
	public function process (array $arguments = []) {
		...
		if (true or (\strlen($api_key) > 0)) { # no API key needed
			
			# load data from URL
			$curl_handle = \curl_init();
			
			$url = 'http://ip-api.com/json/' . \urlencode($ip); # . '?access_key=' . \urlencode($api_key);
			...
			if (isset($decoded->country)) {
				$result->country = $decoded->country;
			}
