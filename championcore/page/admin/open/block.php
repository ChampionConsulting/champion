...
class Block extends Base {
	...
	protected function handle_get (array $request_params, array $request_cookie) : string {
		...
		$view_model->datum_item = new \championcore\store\block\Item();
		...
		# block piles for move
		$view_model->block_pile_directory = \dirname( $view_model->param_get_f );
		
		$view_model->block_piles = new \championcore\store\block\Pile( \championcore\get_configs()->dir_content . '/blocks' );
		
		$view_model->block_piles = \championcore\store\block\Pile::flatten($view_model->block_piles);
		...
		#render
		$view = new \championcore\View( \championcore\get_configs()->dir_template . '/admin/open/block.phtml' );
	...
	protected function handle_post (array $request_params, array $request_cookie) : string {
		...
		try {
			...
			if (    !empty($param_textblock)
				...
				# parameters
				$block_filename = $param_filename;
				
				$path_info = \pathinfo( $block_filename );
				
				$block_filename = \str_replace( ('.' . $path_info['extension']), '', $block_filename );
				$block_filename = \championcore\filter\item_url( $block_filename ) . '.' . $path_info['extension'];
				
				$block_filename_f = \ltrim($block_filename, '/');
				$block_filename_f = \str_replace( ('.' . $path_info['extension']), '', $block_filename_f );
				...
				$block_item->html            = $param_textblock;
				$block_item->meta_searchable = \championcore\filter\yes_no($request_params["meta_searchable"]);
				...
				if (\sizeof($param_meta_access_user_group) > 0) {
					...
					foreach ($param_meta_access_user_group as $selected_group_key => $selected_group_value) {
						if ($selected_group_value == '-') {
							unset($user_group_list->{$selected_group_key}->permissions->block->{$block_filename_f});
						} else {
							$user_group_list->{$selected_group_key}->permissions->block->{$block_filename_f} = 'rw';
						}
					}
				...
				if (($request_params['meta_draft_mode'] == 'yes') and !$is_draft_mode) {
					...
					# bounce
					\header("Location: index.php?p=open&f=" . \dirname($block_filename_f) . '/draft-' . \basename($block_filename_f) . "&e={$path_info['extension']}");
					exit;
				}
				...
				if (($request_params['meta_draft_mode'] == 'no') and $is_draft_mode) {
					...
					\header("Location: index.php?p=open&f=" . \dirname($block_filename_f) . "/{$new_basename}&e={$path_info['extension']}");
					exit;
				}
			}
